...
const OpenChat = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:3001/chats/message/", message);
      ...
      for (const memberId of currentChat.members) {
        ...
        if (memberId !== user._id && !socketUsers.some(user => user.userId === memberId)) {
          ...
          await axios.post("http://localhost:3001/chats/message/notify", memberObject);
        }
      }
    } catch (err) {
      console.log(err);
  ...
  useEffect(() => {
    const getConversations = async () => {
      try {
        const response = await fetch(`http://localhost:3001/chats/conversation/` + user._id, {
          method: 'GET'
        });
  ...
  useEffect(() => {
    const getStudentsAndTeachers = async () => {
      try {
        const response = await fetch(`http://localhost:3001/users/StudentsAndTeachers/`, {
          method: 'POST'
        });
        const data = await response.json();
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/chats/message/` + currentChat?._id, {
          method: 'GET'
        });
