  ...
  const handleMessageClick = () => {
    ...
    const deleteMessageNotifications = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3001/chats/message/', {
          method: 'DELETE',
          headers: {
  ...
  const goForSearch = async () => {
    ...
    try {
      ...
      if (type === 'home' || type === undefined || type === 'profile') {
        const response = await fetch('http://localhost:3001/chats/users', {
          method: 'GET'
        ...
        const response = await fetch('http://localhost:3001/chats/groups', {
          method: 'GET'
        });
  ...
  useEffect(() => {
    ...
    const fetchStudentNotifications = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/studentnotification/${user._id}`);
        if (!response.ok) {
